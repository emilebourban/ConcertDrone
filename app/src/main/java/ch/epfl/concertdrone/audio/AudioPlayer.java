package ch.epfl.concertdrone.audio;
...
public class AudioPlayer {
    ...
    public synchronized void start() {
        Log.i(TAG, "entering start Class AudioPlayer");
        if (!mPlaying) {
            mPlaying = true;
            if (mAudioTrack != null) {
                mAudioTrack.play();
    ...
    public synchronized void stop() {
        Log.i(TAG, "entering stop Class AudioPlayer");
        if (mPlaying) {
            mPlaying = false;
            if (mAudioTrack != null) {
                mAudioTrack.pause();
    ...
    public synchronized void release() {
        Log.i(TAG, "entering release Class AudioPlayer");
        mPlaying = false;
    ...
    public void configureCodec(int sampleRate) {
        Log.i(TAG, "entering configureCodec Class AudioPlayer");
        if (mAudioTrack != null) {
            mAudioTrack.flush();
        ...
        synchronized (AudioPlayer.this) {
            Log.i(TAG, "entering AudioPlayer Class AudioPlayer");
            if (mPlaying) {
                mAudioTrack.play();
            }
        }
    ...
    public void onDataReceived(ARNativeData currentFrame) {
        Log.i(TAG, "entering onDataReceived Class AudioPlayer");
